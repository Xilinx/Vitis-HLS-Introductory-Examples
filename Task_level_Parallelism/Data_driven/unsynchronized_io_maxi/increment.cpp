...
#include <iostream>
void process_11(hls::stream<trans_pkt> &input, int *out,
                hls::stream<int> &int_fifo) {
...
void process_23(hls::stream<int> &int_fifo, int *in,
...
#pragma HLS PIPELINE II = 1 style = flp
    ...
    std::cout << "value of in1 " << in[i] << " value of val is " << val
              << std::endl;
    t_out.data = in2;
    output.write(t_out);
  }
...
extern "C" {
void increment(hls::stream<trans_pkt> &input, hls::stream<trans_pkt> &output,
               int *out, int *in) {
#pragma HLS DATAFLOW
