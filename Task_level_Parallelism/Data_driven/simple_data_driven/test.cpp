...
void odds_and_evens(hls::stream<int> &in, hls::stream<int> &out1,
  ...
  hls_thread_local hls::task t1(
      splitter, in, s1,
      s2); // t1 infinitely runs func1, with input in and outputs s1 and s2
  hls_thread_local hls::task t2(
      odds, s1,
      out1); // t2 infinitely runs func2, with input s1 and output out1
  hls_thread_local hls::task t3(
      evens, s2,
      out2); // t3 infinitely runs func3, with input s2 and output out2
}
