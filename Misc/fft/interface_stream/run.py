...
client.set_workspace(path='./work_interface_stream')

# Delete the component if it already exists
if os.path.exists('./work_interface_stream/component_interface_stream'):
	client.delete_component(name='component_interface_stream')

# Create component. Create new config file in the component folder of the workspace
comp = client.create_hls_component(name='component_interface_stream', cfg_file = ['hls_config.cfg'], template = 'empty_hls_component')

# Get handle of config file, then programmatically set desired options
cfg_file = client.get_config_file(path = './work_interface_stream/component_interface_stream/hls_config.cfg')
...
cfg_file.set_value (section = 'hls', key = 'clock',                 value = '2.5') # 400MHz
...
# Run flow steps
comp = client.get_component(name='component_interface_stream')
