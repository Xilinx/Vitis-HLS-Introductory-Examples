...
if os.path.exists('./work_interface_array/comp_interface_array'):
	client.delete_component(name='comp_interface_array')

# Create component. Create new config file in the component folder of the workspace
comp = client.create_hls_component(name='comp_interface_array', cfg_file = ['hls_config.cfg'], template = 'empty_hls_component')

# Get handle of config file, then programmatically set desired options
cfg_file = client.get_config_file(path = './work_interface_array/comp_interface_array/hls_config.cfg')
...
# Run flow steps
comp = client.get_component(name='comp_interface_array')
