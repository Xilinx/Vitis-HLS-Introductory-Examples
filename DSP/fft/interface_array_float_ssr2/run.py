...
client.set_workspace(path='./work_interface_array_float_ssr2')

# Delete the component if it already exists
if os.path.exists('./work_interface_array_float_ssr2/comp_interface_array_float_ssr2'):
	client.delete_component(name='comp_interface_array_float_ssr2')

# Create component. Create new config file in the component folder of the workspace
comp = client.create_hls_component(name='comp_interface_array_float_ssr2', cfg_file = ['hls_config.cfg'], template = 'empty_hls_component')

# Get handle of config file, then programmatically set desired options
cfg_file = client.get_config_file(path = './work_interface_array_float_ssr2/comp_interface_array_float_ssr2/hls_config.cfg')
...
# Run flow steps
comp = client.get_component(name='comp_interface_array_float_ssr2')
