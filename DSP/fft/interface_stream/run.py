...
if os.path.exists('./work_interface_stream/comp_interface_stream'):
	client.delete_component(name='comp_interface_stream')

# Create component. Create new config file in the component folder of the workspace
comp = client.create_hls_component(name='comp_interface_stream', cfg_file = ['hls_config.cfg'], template = 'empty_hls_component')

# Get handle of config file, then programmatically set desired options
cfg_file = client.get_config_file(path = './work_interface_stream/comp_interface_stream/hls_config.cfg')
...
cfg_file.set_values(section = 'hls', key = 'tb.file',  values = [cwd+'fft_tb.cpp', cwd+'data'])
...
# Run flow steps
comp = client.get_component(name='comp_interface_stream')
