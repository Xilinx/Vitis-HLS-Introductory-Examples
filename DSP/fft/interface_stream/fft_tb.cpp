...
int main()
{
    ...
    hls::stream<cmpxDataIn> xn_input;
    hls::stream<cmpxDataOut> xk_output;
    ...
    for (int frame = 0; frame < SIM_FRAMES; ++frame)
    {
        ...
        sprintf(dat_filename, "data/stimulus_%02d", frame);
        ...
        else
        {
            ...
            while (fgetc(stimfile) != EOF && line_no < SAMPLES+5)
            {
                ...
                default:
                    ...
                    xn_input.write(cmpxDataIn(input_data_re, input_data_im));
        ...
        //status_t st;
        bool st;
        fft_top(FWD_INV, sc_sch, xn_input, xk_output, &st);

	// Open golden results .res file for reading 
        FILE* resfile;
	sprintf(res_filename, "data/stimulus_%02d", frame);
	...
	if ((resfile = fopen(res_filename, "r")) == 0)
	{
        ...
        for (int i = 0; i < (1<<NFFT); i++)
        {
            ...
            xk_out = xk_output.read();
            data_out_t golden = dummy_re;
            if (golden != xk_out.real())
            {
                error_num++;
                cout << "Frame:" << frame << " index: " << i 
                     << "  Golden: " <<  golden.to_float() << " vs. RE Output: " << setprecision(14) << xk_out.real().to_float() << endl;
            }
            golden = dummy_im;
            if (golden != xk_out.imag())
            {
                error_num++;
                cout << "Frame:" << frame << " index: " << i 
                     << "  Golden: " << golden.to_float() << " vs. IM Output: " << setprecision(14) << xk_out.imag().to_float() << endl;
            }
        }
        fclose(resfile);
    }
