...
int main() {
    ...
    hls::vector<cmpxDataIn, FFT_SSR> in_tmp;
    hls::vector<cmpxDataOut, FFT_SSR> out_tmp;
    ...
    for (int frame = 0; frame < SIM_FRAMES; ++frame) {
        ...
        for(int i = 0; i < SAMPLES; i += FFT_SSR){
            for (int j = 0; j < FFT_SSR; j++) {
                in_tmp[j] = xn_input[i + j];
            }
            in.write(in_tmp);
        }
        fft_top(FWD_INV, in, out, &ovflo);
        for(int i = 0; i < SAMPLES; i += FFT_SSR){
            out_tmp = out.read(); 
            for (int j = 0; j < FFT_SSR; j++) {
                xk_output[i + j] = out_tmp[j];
            }
        }
